...
multiplo.vector.while <- function(v, numero){
  ...
  while(i<length(v)){
  i = i+1
  multiplo.w = numero%%v[i] ==0 & numero >=v[i]
  print(multiplo.w)}
}

multiplo.vector.for <- function(v, numero){
  for(i in v){
    multiplo.f = numero%%i ==0 & numero >= i
    print(multiplo.f)}}



multiplo.vector.op <- function(v, numero){
  multiplo.op = numero%%v == 0 & numero >= v
  return(multiplo.op)
}
...
stopifnot(multiplo.vector.op(c(7,8,6),12) == TRUE) # no entiendo el porqué sería TRUE, si en el vector (7,8,6), sólo el 6 sería multiplo de 12, y para que se cumpla el TRUE, todos tedrían que cumplirse
...
multk <- function(numero, k){
    multk= seq(from = 0, to = numero, by = k )
    return(multk)
}


multk.lista = sapply(c(17, 21, 23, 25) , FUN = multk, k=4)

multk.max <- function(numero, k,n){
 multi= seq(from = 0, to = numero, by = k )
 return(multi[1:n])
}
...
multk.lista
#devuelve listas porque no puede hacer vectores, ya que poseen diferentes longitudes de resultados


m = sapply(c(17, 21, 23, 25) , FUN = multk.max, k=4,n=3)
multk.matriz = matrix(m, nrow = 3,ncol = 4)
'devuelve una matriz ya que al ser del mismo tamaño R puede compactar la información y la muestra de la forma más visualmente comprensible'
...
ponderada <- function(v){
    ponderada = sum(v**2)/sum(v)
    return(ponderada)
}


m = matrix(data = c(4, -1.2, 46, 78, -2.3, 8, -2.4, 92, 156, -4.6, 12, -3.6, 138, 234, -6.9), ncol = 5)
ponderada.filas = apply(X = m, MARGIN = c(1,2), FUN = ponderada)
# FIN SOLUCION
```
...
mayusculas <- function(v, n){
  if (n<=length(v)){
    ...
    print('Se ha sobrepasado el número de mayúsculas que se pueden transformar')
}}
